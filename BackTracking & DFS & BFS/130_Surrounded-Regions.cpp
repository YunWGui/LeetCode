...
public:
    void solve(vector<vector<char>>& board) {
        ...
        for ( int row = 0; row < rows; ++row ) {
            if ( board[row][0] == 'O' )  // 左边界
                backTrack( row, 0, rows, cols, board );
            if ( board[row][cols - 1] == 'O' )  // 右边界
                backTrack( row, cols - 1, rows, cols, board );
        }
        // 上、下边界
        for ( int col = 1; col < cols - 1; ++col ) {
            if ( board[0][col] == 'O' )  // 上边界
                backTrack( 0, col, rows, cols, board );
            if ( board[rows - 1][col] == 'O' )  // 下边界
                backTrack( rows - 1, col, rows, cols, board );
        }
    ...
    // 将与边界连通的 'O' 标记为 'N'
    void backTrack( int row, int col, int rows, int cols, vector<vector<char>>& board ) {
        ...
        backTrack( row - 1, col, rows, cols, board );  // 左
        backTrack( row + 1, col, rows, cols, board );  // 右
        backTrack( row , col - 1, rows, cols, board );  // 上
        backTrack( row , col + 1, rows, cols, board );  // 下
    }
};
